...
\IEEEPARstart{C}{hronic} Obstructive Pulmonary Disease (COPD) is an umbrella term that describes several pulmonary affections.  They are characterized as a slowly progressive condition marked by airflow limitation. This pathology presents a musculoskeletal atrophy~\cite{Kocsis2016}. In order to carve these after effects a Pulmonary Rehabilitation procedure is a viable treatment for patients.  Pulmonary Rehabilitation procedures consist of controlled walking activities and physical exercises that patients perform under the supervision of a physical therapist.  However, COPD patients present a severe low oxygen saturation illness and they require oxygen supply, particularly when performing physical activities.  Hence, they are required to carry an oxygen tank for the oxygenotherapy assistance, but their own condition hinders their ability to precisely carry the often bulky external tank. 
...
\caption{Side-view of the robot prototype.}
\label{fig:alpibot1}
\end{figure}
...
\noindent where $ \phi $ is the free parameter, $a$ is the limit length of the arc of the curve, and $x$ and $y$ are the parametric functions that determine the shape of the trajectory on the navigation plane. Similar curves are also used in other proposed experiments in \cite{Neto2015,Endo2015}.  


We propose the following metrics to evaluate the performance of the solution:
\begin{itemize}
...
A model of the proposed design was first built on Webots simulator.  The threading mechanism was implemented using virtual threads.  The leader traveled according to a predefined trajectory with constant velocity, following the curve trajectory.
...
Metric evaluation results for the simulation and the real world experiment are shown in Table~\ref{tab:simulationmetricsftt} and Table~\ref{tab:cap_ft_npd_table}.
...
\begin{center}
\captionof{table}{Total trajectory deviation t.t.d. for different algorithm constants (simulation).}
\begin{tabular}{ |c|c|c| }
\hline
$c_v$ & $c_{\alpha}$ & t.t.d. \\
\hline
15  &   5  & 2.055\\
\textbf{15}  &   \textbf{10}  & \textbf{1.0902}\\
15  &   15  & 1.5059\\
\hline
\end{tabular}
...
\begin{center}
\captionof{table}{Total trajectory deviation t.t.d. for different algorithm constants (real world).}
\begin{tabular}{ |c|c|c| }
\hline
$c_v$ & $c_{\alpha}$ & t.t.d. \\
\hline
\textbf{25}  &   \textbf{20}  & \textbf{1.9761}\\
25  &   35 & 2.3528\\
\hline
\end{tabular}
...
\begin{center}
\captionof{table}{Robot-leader distance over time in the motion capture experiments.}
\begin{tabular}{|c|c|c|c|}\hline
$c_v$ & $c_{\alpha}$ & Avg. distance & Max distance \\
\hline
25 & 20 & 0.6561m &   0.7916m \\
25 & 35 & 0.6843m &   0.8445m  \\
\hline
\end{tabular}
\label{tab:cap_ft_vs_rg_dist_table}
\end{center}
...
%   \subfloat[ \label{rw1a}]{%
    \\
  \caption{Experimentation Results: Trajectories of the leader and follower in motion capture environment.}
...
ALPI is a non-profit civil association located in Buenos Aires, Argentina, that provides neuromotor rehabilitation for pediatric and adult patients. Four professional care-givers from ALPI  were invited to test and evaluate the controlling strategy on the prototype.  A live demonstration of the robot working and following a moving person was performed.
...
In our experiments, we demonstrate that the strategy proposed achieves a following behavior. With the right parameters, the robot was never further that 0.79m away from the followed subject, showing results comparable to those obtained in computer simulations. Feedback from medical professionals was also positive, reinforcing the viability of this solution for real world use. 
...
It is essential to involve stakeholders such as patients, doctors, nurses, and any other professionals involved in the rehabilitation process early in the design roadmap to have a full understanding of the problem and the product needed to solve it.

% They are the ones who understand the problem better than anyone else, and will be the end users of any developed product, as long as it is useful for them.
